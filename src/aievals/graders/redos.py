...
**Vendored**, deliberately. This file is a copy of
``src/mcp_devserver/tools/redos.py`` from
https://github.com/kogunlowo123/mcp-developer-server at commit ``bfdb362``,
adapted only in its module docstring and its exception type. It is duplicated
rather than depended on because a four-dependency evaluation harness should not
acquire a Model Context Protocol server to obtain two hundred lines of pure
static analysis, and because the two copies are free to diverge as each
project's threat model does.

The problem it solves here: a suite file may declare a ``regex`` grader, and a
suite file is untrusted input. ``(a+)+b`` against forty ``a`` characters is a
single ``re.search`` that does not return in the lifetime of the universe, and
no timeout above it helps — ``asyncio.wait_for`` around ``asyncio.to_thread``
abandons the coroutine but cannot stop the thread, and Python offers no way to
interrupt a running ``re`` match. The thread keeps a core busy for the life of
the process, which in CI is a job that burns its whole time budget and reports
nothing.

So the check happens *before* the pattern is compiled, and it is static. This is
a **heuristic and not a decision procedure** — recognising every exponential
regular expression is undecidable — but it catches the shapes that appear in
practice:
...
merely slow is bounded only by the per-case timeout.
"""
